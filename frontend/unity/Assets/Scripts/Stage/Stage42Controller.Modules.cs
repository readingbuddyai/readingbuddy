...
public partial class Stage42Controller
{
    ...
    [Tooltip("튜토리얼 이후 입력을 기다릴지 여부")] public bool requireTriggerAfterTutorial = false;
    ...
    {
        public string reply;
        public bool isReplyCorrect;
        public float accuracy;
        public string audioUrl;
    }

    [Serializable]
    private class VoiceReplyResp
    {
        public bool success;
        public string message;
        public VoiceReplyData data;
    }
    ...
    private void ConfigureTutorialController()
    {
        ...
        _tutorialDependencies.ProgressText = null;
        ...
        _tutorialDependencies.CorrectSfx = clipCorrectPlaced;
        ...
        _tutorialController.guide3DCharacter = null;
    ...
    private System.Collections.IEnumerator Co_TutorialToggleChoices(bool show)
    {
        ...
        if (consonantChoicesContainer) consonantChoicesContainer.SetActive(show);
        if (vowelChoicesContainer) vowelChoicesContainer.SetActive(show);
        yield break;
    }
    ...
    private IEnumerator FetchQuestionsWithSession(Action<List<QuestionDto>> onDone)
    {
        ...
        if (result == null || !result.Success)
        {
            if (logVerbose && result != null)
                Debug.LogWarning($"[Stage42] set 실패: code={result.ResponseCode} body={result.RawBody}");
            onDone?.Invoke(null);
            yield break;
        }
        ...
        try
        {
            if (logVerbose) Debug.Log($"[Stage42] set 응답: {result.RawBody}");
    ...
    private IEnumerator StartStageSessionWithSession()
    {
        ...
        {
            Debug.LogWarning($"[Stage42] stage/start 실패 또는 stageSessionId 미수신: code={result.ResponseCode} body={result.RawBody}");
        }
    }
    ...
    private IEnumerator UploadVoiceWithSession(string answerValue, byte[] wavData, Action<bool> onDone)
    {
        var session = ConfigureSessionController();
        StageSessionController.VoiceCheckResult result = null;
        yield return session.CheckVoice(stageSessionId, stageTwoPart, _currentProblemNumber, answerValue, wavData, r => result = r);

        if (result == null || !result.Success)
        {
            if (logVerbose && result != null)
                Debug.LogWarning($"[Stage42] check/voice 실패: code={result.ResponseCode} body={result.RawBody}");
            onDone?.Invoke(false);
            yield break;
        }

        if (logVerbose) Debug.Log($"[Stage42] check/voice 응답: {result.RawBody}");
        bool ok = false;
        try
        {
            var parsed = JsonUtility.FromJson<VoiceReplyResp>(result.RawBody ?? string.Empty);
            ok = parsed?.data?.isReplyCorrect ?? false;
        }
        catch (Exception e)
        {
