...
PHASE_CONFIG = {
    1: {
        ...
        'learning_rate': 5e-5,
        'warmup_steps': 2000,
        'target_wer': 0.12,  # 목표: PER < 12%
        'augmentation_strength': 'weak',
    },
}

# 예상 학습 시간:
# - 1,771,800 샘플 × 3 epochs = 5,315,400 샘플
# - batch_size 16, ~332k steps
# - 예상 시간: ~10-14일 (GPU 3 기준)

# 공통 설정
COMMON_CONFIG = {
    'gpu': '1',
    'batch_size': 8,  # 16→8 (메모리 절약)
    'gradient_accumulation_steps': 2,  # 1→2 (동일한 effective batch size 유지)
    'weight_decay': 0.01,
    'gradient_clip': 1.0,
    ...
    'output_base_dir': '/home/j-k13a206/fine_tunining_new/checkpoints_full',
    ...
    # Early Stopping
    'early_stopping_patience': 30,  # 3 → 30 (대규모 데이터 대응) ⭐
...
def run_phase(
    ...
    val_dataset = StreamingChildSpeechDataset(
        ...
        processor=processor,
        g2p=g2p,
        max_samples=20000,  # Validation 20,000개로 제한 (r=32와 동일) ⭐
    )
    ...
    training_args = TrainingArguments(
        ...
        eval_strategy="steps",
        eval_steps=2000,           # 500 → 2000 (r=32와 동일) ⭐
        save_steps=2000,           # 500 → 2000 (r=32와 동일) ⭐
        logging_steps=50,
...
def main():
    ...
    if args.phase == 'all':
        ...
        if phase_num not in PHASE_CONFIG:
            print(f"❌ 오류: Phase {phase_num}는 존재하지 않습니다.")
            print(f"   사용 가능한 Phase: {list(PHASE_CONFIG.keys())}")
            sys.exit(1)
