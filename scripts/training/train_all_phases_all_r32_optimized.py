...
# -*- coding: utf-8 -*-
"""
Wav2Vec2-XLS-R-300M 어린이 음성 파인튜닝 - r=32 최적화 버전
LoRA r=32 + Validation 최적화 (빠른 학습!)
"""
...
PHASE_CONFIG = {
    1: {
        'name': 'phase1_full_3500h_r32',
        'train_ratio': 1.0,  # 전체 1.7M 데이터 사용
        'epochs': 10,
        'learning_rate': 5e-5,
        'warmup_steps': 2000,
        'target_wer': 0.10,  # 목표: PER < 10% (r=32는 더 강력!)
...
COMMON_CONFIG = {
    ...
    # 경로
    'data_dir': '/home/j-k13a206/data/child_extracted',
    'model_path': '/home/j-k13a206/models/wav2vec2-korean-phoneme',
    'output_base_dir': '/home/j-k13a206/fine_tunining_new/checkpoints_full_r32',  # 경로 구분 ⭐

    # LoRA 설정 - r=32 버전 ⭐⭐⭐
    'lora_r': 32,        # 16 → 32 (2배 증가)
    'lora_alpha': 64,    # 32 → 64 (2배 증가)
    'lora_dropout': 0.1,
    'freeze_layers': list(range(0, 8)),  # 0~7층 동결

    # Early Stopping
    'early_stopping_patience': 30,  # None → 30 ⭐
    'early_stopping_threshold': 0.005,

    # Validation 최적화 ⭐⭐⭐
    'max_val_samples': 20000,  # Validation 20,000개만 사용 (6배 빠름!)
}
...
class DetailedLoggingCallback(TrainerCallback):
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        with open(self.log_file, 'w', encoding='utf-8') as f:
            f.write(f"# {self.phase_name} Training Log (r=32 Optimized)\n\n")
...
def run_phase(
    ...
    print("=" * 80)
    print(f"🚀 Phase {phase_num} 시작: {phase_name} (r=32 최적화 버전)")
    print("=" * 80)
    ...
    # Validation 최적화: 20,000개만 사용 ⭐⭐⭐
    val_dataset = StreamingChildSpeechDataset(
        data_dir=config['data_dir'],
        split="2.Validation_split",
        processor=processor,
        g2p=g2p,
        max_samples=config['max_val_samples'],  # 20,000개만!
    )
    ...
    training_args = TrainingArguments(
        ...
        eval_strategy="steps",
        eval_steps=2000,           # 500 → 2000 (4배 감소) ⭐
        save_steps=2000,           # 500 → 2000 (4배 감소) ⭐
        logging_steps=50,
    ...
    # Callbacks
    callbacks = [
        # EarlyStoppingCallback 제거 - epoch 10까지 전부 학습
    ...
    # 학습 시작
    print(f"\n🏃 Phase {phase_num} 학습 시작! (r=32)")
    print(f"  목표 WER: < {phase_config['target_wer']*100:.0f}%")
    print(f"  LoRA Rank: {config['lora_r']}")
    print(f"  Validation 샘플: {config['max_val_samples']:,}개")
    print(f"  Eval/Save 간격: 2000 steps")
    print(f"  예상 평가 시간: ~2.5분/회 (vs 14.5분/회)")
    print("-" * 80)
...
# =====================
def main():
    parser = argparse.ArgumentParser(description="Wav2Vec2 r=32 Optimized Training")
    parser.add_argument(
        '--phase',
        type=str,
        default='1',
        help='실행할 Phase: 1 (기본값: 1)'
    )
    parser.add_argument(
        '--resume_from',
        type=str,
    ...
    print("\n" + "=" * 80)
    print("🎯 Wav2Vec2-XLS-R-300M 어린이 음성 파인튜닝 (r=32 최적화)")
    print("=" * 80)
    print(f"실행 Phase: {args.phase}")
    print(f"GPU: {args.gpu}")
    print(f"LoRA Rank: {COMMON_CONFIG['lora_r']}")
    print(f"Validation 샘플: {COMMON_CONFIG['max_val_samples']:,}개")
    print(f"데이터: {COMMON_CONFIG['data_dir']}")
    print("=" * 80 + "\n")

    phase_num = int(args.phase)

    if phase_num not in [1]:
        print("❌ 오류: Phase는 1이어야 합니다.")
        sys.exit(1)
