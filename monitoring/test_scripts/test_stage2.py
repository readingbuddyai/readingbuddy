...
# Labels: stage(2), endpoint(start/set/attempt/complete), status(success/fail)
api_request_total = Counter(
    'stage2_api_request_total',
    'Total API requests for Stage 2',
...
api_request_duration = Histogram(
    'stage2_api_request_duration_seconds',
    'API request duration for Stage 2',
    ['stage', 'endpoint']
)
...
api_success_rate = Gauge(
    'stage2_api_success_rate',
    'API success rate for Stage 2',
    ['stage']
)

# Counter: 생성된 문제 개수를 카운트
problem_count = Counter(
    'stage2_problem_generated_total',
    'Total problems generated for Stage 2',
    ['stage']
)

# Counter: 문제 시도 횟수 카운트
attempt_total = Counter(
    'stage2_attempt_total',
    'Total problem attempts for Stage 2',
    ['stage', 'is_correct']
)

# Gauge: 문제 시도 정답률
attempt_accuracy = Gauge(
    'stage2_attempt_accuracy',
    'Problem attempt accuracy for Stage 2',
    ['stage']
)
...
class TrainTester:
    ...
    def __init__(self):
        ...
        # 스테이지별 성공/실패 통계
        self.stats = {
            '2': {'success': 0, 'fail': 0, 'attempt_correct': 0, 'attempt_wrong': 0}
        }
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                for i, prob in enumerate(problems, 1):
                    ...
                    word_length = prob.get('wordLength', 0)
                    print(f"  문제 {i}: '{problem_word}' (길이: {word_length})")
    ...
    def run_basic_test(self):
        """
        기본 기능 테스트: Stage 2를 1회 테스트
        """
        self.print_header("기본 기능 테스트 시작")
        self.test_stage('2', config.TEST_CONFIG['stage_2_count'])
    ...
    def print_summary(self):
        ...
        if total > 0 or total_attempts > 0:
            print(f"\n{Fore.CYAN}Stage {stage} (단어 음절 수 세기):{Style.RESET_ALL}")
        ...
        print(f"  확인 가능한 메트릭:")
        print(f"    - stage2_api_request_total (요청 총 횟수 - endpoint별)")
        print(f"    - stage2_api_request_duration_seconds (응답 시간 분포)")
        print(f"    - stage2_api_success_rate (API 성공률)")
        print(f"    - stage2_problem_generated_total (생성된 문제 수)")
        print(f"    - stage2_attempt_total (문제 시도 횟수 - 정답/오답별)")
        print(f"    - stage2_attempt_accuracy (문제 시도 정답률)")
